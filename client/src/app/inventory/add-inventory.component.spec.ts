...
describe('AddInventoryComponent', () => {
  ...
  describe('The guardian name field', () => {
    ...
    beforeEach(() => {
      guardianNameControl = addInventoryComponent.addInventoryForm.controls.guardianName;
    });
  ...
  describe('Students FormArray', () => {
    ...
    it('should start with an empty students array', () => {
      const students = addInventoryComponent.students;
      expect(students).toBeDefined();
      expect(students.length).toBe(0);
    });

    it('should add a student when addStudent() is called', () => {
      addInventoryComponent.addStudent();
      const students = addInventoryComponent.students;
    ...
    it('should remove a student when removeStudent() is called', () => {
      addInventoryComponent.removeStudent(0);
      const students = addInventoryComponent.students;
    ...
    it('when all required fields are valid, the the whole form should be valid', () => {
      addInventoryForm.controls.guardianName.setValue('Chris Smith');
      addInventoryForm.controls.address.setValue('123 Avenue');
      addInventoryForm.controls.timeSlot.setValue('9:00-10:00');
      addInventoryForm.controls.email.setValue('csmith@email.com');

      addInventoryComponent.addStudent();
      const student = addInventoryComponent.students.at(0);
  ...
  describe('The address field', () => {
    ...
    beforeEach(() => {
      addressControl = addInventoryComponent.addInventoryForm.controls.address;
    });
  ...
  describe('The email field', () => {
    ...
    beforeEach(() => {
      emailControl = addInventoryComponent.addInventoryForm.controls.email;
    });
...
describe('AddInventoryComponent#submitForm()', () => {
  ...
  beforeEach(() => {
    ...
    component.addInventoryForm.controls.guardianName.setValue('Chris Smith');
    component.addInventoryForm.controls.address.setValue('123 Avenue');
    component.addInventoryForm.controls.timeSlot.setValue('9:00-10:00');
    component.addInventoryForm.controls.email.setValue('csmith@email.com');
    //component.addInventoryForm.controls.student.setValue('admin');
  });
  ...
  it('should call addInventory() and handle error response', () => {
    ...
    // "Spy" on the `.addInventory()` method in the inventory service. Here we basically
    ...
    component.submitForm();
    // Check that `.addInventory()` was called with the form's values which we set
    // up above.
    expect(addInventorySpy).toHaveBeenCalledWith(component.addInventoryForm.value);
  ...
  it('should transform requestedSupplies string into trimmed array', () => {
    const studentsArray = component.addInventoryForm.get('students') as FormArray;
    ...
    component.addInventoryForm.patchValue({
      students: [{
        name: 'John',
        grade: '5',
        school: 'ABC',
    ...
    component.submitForm();
    expect(addInventorySpy).toHaveBeenCalledWith(
